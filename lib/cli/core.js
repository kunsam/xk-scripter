...
var _dtdoc = require("../dtdoc.config");
...
program.command("helpdoc").description("show xksciprt doc").action(function () {
...
function recursiveGetResult(clist) {
  clist.forEach(function (target, index) {
    console.log(_chalk2.default.white(index + 1 + ". " + target.name));
  });
  var chosenIndex = (0, _loopInput.loopInput)("请选择：", function (input) {
  ...
  if (clist[chosenIndex]) {
    if (clist[chosenIndex].link) {
      _chalk2.default.green(clist[chosenIndex].link);
...
program.command("dtdoc").description("show dt develop doc").action(function () {
  console.log(_chalk2.default.magenta("\n-- [\u6587\u6863\u5217\u8868]:"));
  _dtdoc.DTDOC_LINK.forEach(function (target, index) {
    console.log(_chalk2.default.white(index + 1 + ". " + target.name));
  });
  var chosenIndex = (0, _loopInput.loopInput)("请选择：", function (input) {
    var choose = input && parseInt(input);
    if (choose && choose > 0 && choose <= _dtdoc.DTDOC_LINK.length) return choose - 1;
  });
  if (_dtdoc.DTDOC_LINK[chosenIndex]) {
    if (_dtdoc.DTDOC_LINK[chosenIndex].link) {
      _chalk2.default.green(_dtdoc.DTDOC_LINK[chosenIndex].link);
    } else if (_dtdoc.DTDOC_LINK[chosenIndex].children) {
      recursiveGetResult(_dtdoc.DTDOC_LINK[chosenIndex].children);
    }
  }
});

program.command("dt").description("run a new dt project!").action(function () {
